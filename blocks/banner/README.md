...
A full-width banner with an image, a bold title, and an optional call-to-action button, styled with a solid white background and black text by default.
...
Add `(dark)` after the block name to swap the background from white to a dark neutral (with white text):
...
- **Image**: any image works; it's shown in full (no cropping) inside its own container — letterboxed on a light background if its aspect ratio doesn't match the container.
...
- `--banner-background-color` (default `#fff`)
- `--banner-text-color` (default `#000`)
